...
# Paths
input_path = os.path.join(EPG_ROOT, 'qatar4.xml')
...
def main():
    print("*****************Qatar4_iet5 EPG******************")
    sys.stdout.flush()
    print("Downloading Qatar4_iet5 EPG guide\nPlease wait....")
    sys.stdout.flush()
    try:
        # Download the XML file
        response = requests.get('https://www.open-epg.com/files/qatar4.xml', verify=False)
        if response.status_code == 200:
            with io.open(input_path, 'w', encoding="utf-8") as f:
                f.write(response.text)
            print("##########################################")
            print("qatar4.xml Downloaded Successfully")
            print("##########################################")
            ...
            print("Failed to download /qatar4.xml. Status code: {}".format(response.status_code))
    except requests.exceptions.RequestException as e:
        print("Failed to download /qatar4.xml: {}".format(e))
...
def rename_file():
    ...
    print("qatar4.xml file successfully created")
    print("############################################################")
    print("The time is set to +0200 ,and if your time is different,")
    print("you can modify the qatar4iet5.py file at the following path:")
