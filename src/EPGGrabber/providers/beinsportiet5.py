...
# Paths
path = os.path.join(EPG_ROOT, 'qatarpremium.xml')
path_1 = os.path.join(EPG_ROOT, 'out.xml')

def main():
    print("*****************Bein_Sport_iet5 EPG******************")
    sys.stdout.flush()
    print("Downloading Bein_Sport_IET5 EPG guide\nPlease wait....")
    sys.stdout.flush()

    try:
        # Download the XML file
        response = requests.get('https://www.bevy.be/bevyfiles/qatarpremium.xml', verify=False)
        if response.status_code == 200:
            ...
            # Change qatarpremium.xml format to elcinema format
...
def rename_file():
    ...
    print("Cool .... congratulations your qatarpremium.xml file is created - successfully done")
    print("############################################################")
    print("The time is set to +0300, and if your time is different,")
    print("you can modify the beinsportiet5.py file in the following")
...
def update_providers():
    with open(PROVIDERS_ROOT, 'r') as f:
        ...
        for channel in data['bouquets']:
            if channel["bouquet"] == "beinsportiet5":
