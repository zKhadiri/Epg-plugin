...
# Paths
path = os.path.join(EPG_ROOT, 'uaepremium2.xml')
path_1 = os.path.join(EPG_ROOT, 'out.xml')

def main():
    print("*****************UAE_iet5_AR EPG******************")
    sys.stdout.flush()
    print("Downloading UAE_iet5_AR EPG guide\nPlease wait....")
    sys.stdout.flush()

    try:
        # Download the XML file
        response = requests.get('https://www.bevy.be/bevyfiles/uaepremium2.xml', verify=False)
        if response.status_code == 200:
            with io.open(path, 'w', encoding="utf-8") as f:
                f.write(response.text)
            print("##########################################")
            print("uaepremium2.xml Downloaded Successfully")
            print("##########################################")
            ...
            # Change uaepremium2.xml format to elcinema format
            ...
            print("Failed to download /uaepremium2.xml. Status code: {}".format(response.status_code))
    except requests.exceptions.RequestException as e:
        print("Failed to download /uaepremium2.xml: {}".format(e))
...
def rename_file():
    ...
    print("Cool .... congratulations your uaepremium2.xml file is created - successfully done")
    print("############################################################")
    print("The time is set to +0300, and if your time is different,")
    print("you can modify the uaeariet5.py file in the following")
...
def update_providers():
    with open(PROVIDERS_ROOT, 'r') as f:
        ...
        for channel in data['bouquets']:
            if channel["bouquet"] == "uaeariet5.py":
