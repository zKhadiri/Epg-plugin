...
# Paths
input_path = os.path.join(EPG_ROOT, 'poland2.xml')
...
def main():
    print("*****************Poland2_iet5 EPG******************")
    sys.stdout.flush()
    print("Downloading Poland2_iet5 EPG guide\nPlease wait....")
    sys.stdout.flush()
    try:
        # Download the XML file
        response = requests.get('https://www.open-epg.com/files/poland2.xml', verify=False)
        if response.status_code == 200:
            with io.open(input_path, 'w', encoding="utf-8") as f:
                f.write(response.text)
            print("##########################################")
            print("poland2.xml Downloaded Successfully")
            print("##########################################")
            ...
            print("Failed to download /poland2.xml. Status code: {}".format(response.status_code))
    except requests.exceptions.RequestException as e:
        print("Failed to download /poland2.xml: {}".format(e))
...
def rename_file():
    ...
    print("poland2.xml file successfully created")
    print("############################################################")
    print("The time is set to +0200 ,and if your time is different,")
    print("you can modify the poland2iet5.py file at the following path:")
...
def update_providers():
    with open(PROVIDERS_ROOT, 'r') as f:
        ...
        for channel in data['bouquets']:
            if channel["bouquet"] == "Poland2iet5":
