...
CULTURE = 'ar-AE'  # Arabic OSN API culture
DAYS_TO_GRAB = 7
REQUEST_TIMEOUT = 15
MAX_RETRIES = 2
...
# =========================================================
# Fixed OSN Arabic 48-channel target list
# =========================================================
TARGET_48 = [
    'OSNtv One',
    'OSNtv Showcase',
...
def stop_from_duration(start, details):
    ...
    duration = clean_text(first_value(details, [
        'DurationTimeAr', 'DurationTime', 'durationTime'
    ]))
...
def programme_from_item(item):
    ...
    title = clean_text(first_value(item, [
        'Arab_Title', 'ArabicTitle', 'TitleAr', 'TitleAR', 'Title', 'title'
    ]))
    ...
    if details:
        title = clean_text(first_value(details, [
            'Arab_Title', 'ArabicTitle', 'TitleAr', 'TitleAR',
            'EpisodeAr', 'Title'
        ], title)) or title

    desc = clean_text(first_value(details, [
        'Arab_Synopsis', 'ArabicSynopsis', 'SynopsisAr', 'SynopsisAR', 'Synopsis', 'Description'
    ]))
    if not desc:
    ...
    category = clean_text(first_value(details, [
        'GenreArabicName', 'GenreArabName', 'ArabicGenreName', 'GenreName', 'Genre'
    ]))
    ...
    with io.open(xml_file, 'w', encoding='utf-8') as f:
        ...
        for ch in channels:
            ...
            for i, p in enumerate(programmes):
                ...
                f.write(
                    u'    <title lang="ar">%s</title>\n'
                    % xml_escape(p.get('title', ''))
                )
                f.write(
                    u'    <desc lang="ar">%s</desc>\n'
    ...
    if DEBUG:
        try:
            ...
            with io.open(DEBUG_LOG, 'w', encoding='utf-8') as lf:
                lf.write(u'OSN ARABIC EPG DEBUG LOG\n')
