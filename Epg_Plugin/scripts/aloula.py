    ...
    try:
        for url in urls:
            ...
            for elem, next_elem in zip(times, times[1:] + [times[0]]):
                ...
                time_chan.append(2 * ' ' + '<programme start="' + starttime + ' '+time_offset+'" stop="' + endtime + ' '+time_offset+'" channel="Aloula.ma">' + '\n')
    ...
    try:
        for url in urls:
            ...
            for elem, next_elem in zip(time_ar, time_ar[1:] + [time_ar[0]]):
                ...
                time_chan.append(2 * ' ' + '<programme start="' + starttime + ' '+time_offset+'" stop="' + endtime + ' '+time_offset+'" channel="Arriadia.ma">' + '\n')
        ...
        for url in urls:
            ...
            with requests.Session() as s:
                ...
                for elem, next_elem in zip(time_2m, time_2m[1:] + [time_2m[0]]):
                    ...
                    time_chan.append(2 * ' ' + '<programme start="' + starttime + ' '+time_offset+'" stop="' + endtime + ' '+time_offset+'" channel="2m.ma">' + '\n')
    ...
    try:
        ...
        for u, w in zip(urls, week):
            ...
            for elem, next_elem in zip(time_medi, time_medi[1:] + [time_medi[0]]):
                ...
                time_chan.append(2 * ' ' + '<programme start="' + starttime + ' '+time_offset+'" stop="' + endtime + ' '+time_offset+'" channel="Medi1tv.ma">' + '\n')
            for tt, p, d, c in zip(time_chan, prog, desc, cat):
                print(tt + p + d + c)
                with io.open("/etc/epgimport/aloula.xml", "a", encoding='UTF-8')as f:
                    f.write(tt + p + d + c)
    ...
    try:
        for url in urls:
            ...
            for elem, next_elem in zip(times_at, times_at[1:] + [times_at[0]]):
                ...
                starttime = datetime.strptime(date.group() + ' ' + elem[0], '%d/%m/%Y %H:%M').strftime('%Y%m%d%H%M%S')
                endtime = datetime.strptime(date.group() + ' ' + next_elem[0], '%d/%m/%Y %H:%M').strftime('%Y%m%d%H%M%S')
                time_chan.append(2 * ' ' + '<programme start="' + starttime + ' '+time_offset+'" stop="' + endtime + ' '+time_offset+'" channel="athaqafia.ma">' + '\n')
