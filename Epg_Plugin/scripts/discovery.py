...
channels = ['Discovery Central Europe|DCENENG-UTC','Animal Planet Europe|APEUENG-UTC','Discovery Showcase HD|HDEUENG-UTC'
            ,'Discovery Science Europe|SCEUENG-UTC','ID|IDDEENG-UTC','DTX|TUTUROM-UTC']
...
def discovery():
    with requests.Session() as s:
        ...
        for ch in channels:
            ...
            for title,start,end,des,ep,py,dtx_t,dxt_d in zip(titles,date_start,date_end,description,episode,programme_year,dtx_titles,dtx_des):
                if end <=next_update:
                    last_date = end
                    ...
                    if ch.split('|')[0]=="DTX":
                        ...
                        epg+=4*' '+'<desc lang="en">Episode '+ep+' . '+py+' : '+dxt_d.replace('&amp;','and').strip()+'</desc>\n  </programme>\r'
                    else:
                        epg+=2 * ' ' + '<programme start="' + prog_start + ' +0000" stop="' + prog_end + ' +0000" channel="'+ch.split('|')[0]+'">\n'
                        epg+=4*' '+'<title lang="en">'+title.replace('&amp;','and')+'</title>\n'
                        epg+=4*' '+'<desc lang="en">Episode '+ep+' . '+py+' : '+des.replace('&amp;','and').strip()+'</desc>\n  </programme>\r'
                    with io.open("/etc/epgimport/discovery.xml","a",encoding='UTF-8')as f:
                        f.write(epg)
